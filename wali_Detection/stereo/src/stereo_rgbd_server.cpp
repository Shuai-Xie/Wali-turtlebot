...
//kinect::RGBD_Image rgbd_msg;
std_msgs::Header header;

bool get_rgbd_image(kinect::RGBD_Image::Request &req,
                    kinect::RGBD_Image::Response &res) {
...
//            header.seq = transindex;
            cv_bridge::CvImage(header, "bgr8", leftpic).toImageMsg(res.rgb);
            cv_bridge::CvImage(header, "mono16", depthpic).toImageMsg(res.depth);
        } else {
            ROS_INFO("waiting rgbd client...");
        }
    }
}

int main(int argc, char *argv[]) {
    ros::init(argc, argv, "stereo_rgbd_server"); // make this cpp as a node, then no multi_main error!
    ros::NodeHandle n;
    ros::ServiceServer service = n.advertiseService("get_stereo_rgbd_image", get_rgbd_image);

...
static pthread_mutex_t picMutexleft[NUM_THREADS];
static pthread_mutex_t picMutexright[NUM_THREADS];
...
void getImgFromMes(Mat &img_1, Mat &img_2, const int count) {
    ...
    while (1) {
        if (get_pic[cur_get_point].right_has&&get_pic[cur_get_point].left_has) {
            if (pthread_mutex_lock(&(picMutexleft[cur_get_point])) != 0) {
                fprintf(stdout, "lock error!\n");
            }
            if (pthread_mutex_lock(&(picMutexright[cur_get_point])) != 0) {
                fprintf(stdout, "lock error!\n");
            }

            img_1 = get_pic[cur_get_point].img[0].clone();
            img_2 = get_pic[cur_get_point].img[1].clone();
            get_pic[cur_get_point].left_has = false;
            get_pic[cur_get_point].right_has = false;
#ifdef logMess
            cout<<"get      img"<<count<<endl;
#endif

            pthread_mutex_unlock(&(picMutexright[cur_get_point]));//����
            pthread_mutex_unlock(&(picMutexleft[cur_get_point]));//����
...
void *thread_pic(void *thread_mess) {
    ...
    string path_left = "/home/nvidia/wali_ws/src/stereo/img/left/";
    string path_right = "/home/nvidia/wali_ws/src/stereo/img/right/";
    string fileType = ".png";
...
//        cv::imwrite( path_left + strnum + fileType, img_1);
//        cv::imwrite( path_right + strnum + fileType, img_2);
...
//        cv::imshow("left",img_1);
//        cv::imshow("right",img_2);
//        cv::waitKey(1);
...
void *thread_getPic_left(void *thread_mess) {
//    string gst_left = "rtsp://admin:vipa404404@192.168.1.66:554/";
    ...
    if (!leftcapture.open(gst_left)) {
        printf("left camera read error\n");
    }
    int picIndexAll = 0;
    int picIndex = 0;
    cv::Mat leftframe;
...
//        cv::imshow("thread_getPic", leftframe);
//        if (cv::waitKey(1) == 'q') {
//            break;
//        }
        picIndex = picIndexAll % NUM_THREADS;

        if (get_pic[picIndex].left_has) {
#ifdef logMess
        ...
        if (pthread_mutex_lock(&(picMutexleft[picIndex])) != 0) {
            fprintf(stdout, "lock error!\n");
        }

        get_pic[picIndex].img[0] = leftframe.clone();//left img
        get_pic[picIndex].left_has = true;
        pthread_mutex_unlock(&(picMutexleft[picIndex]));
...
void *thread_getPic_right(void *thread_mess) {
//    string gst_right = "rtsp://admin:vipa404404@192.168.1.67:554/";
    ...
    if (!rightcapture.open(gst_right)) {
        printf("right camera read error\n");
    }
    int picIndexAll = 0;
    int picIndex = 0;
    cv::Mat rightframe;
    ...
    while (ros::ok) {
        ...
        if (pthread_mutex_lock(&(picMutexright[picIndex])) != 0) {
            fprintf(stdout, "lock error!\n");
        }
        get_pic[picIndex].img[1] = rightframe.clone(); //right img
        get_pic[picIndex].right_has = true;
        pthread_mutex_unlock(&(picMutexright[picIndex]));
...
//
//        cv::Mat DispRgb;
//        cv::minMaxIdx(depthpic,minp,maxp);
//        cv::applyColorMap((depthpic / maxv * 255),DispRgb,COLORMAP_JET);
...
int main(int argc, char *argv[]) {
    ...
    ros::NodeHandle n;
    chatter_pub = n.advertise<stereo::RGBD_Image>("/stereo/rgbd/image", 1000); // Note: change msg type!
    Get_Remap_Parameters(map11, map12, map21, map22, rectified_roi);

    for (int i = 0; i < NUM_THREADS; i++) {
        get_pic[i].left_has = false;
        get_pic[i].right_has = false;
    }

    trans_pic[0].index = -1;

    // Multi-thread compute disparity
    pthread_t tids[NUM_THREADS + 3];
    int32_t thread_mess[NUM_THREADS + 3];
    int ret;
    ...
    for (int i = 0; i < NUM_THREADS + 3; ++i) {
        thread_mess[i] = i;
        if (i == 0) {
            ret = pthread_create(&tids[i], NULL, thread_getPic_left, (void *) &(thread_mess[i]));
        }
        else if(i == 1)
        {
            ret = pthread_create(&tids[i], NULL, thread_getPic_right, (void *) &(thread_mess[i]));
        }
    ...
    pthread_join(tids[NUM_THREADS + 2], NULL);

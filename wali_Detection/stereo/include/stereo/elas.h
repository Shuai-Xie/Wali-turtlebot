...
private:
	...
	//Interpolation todo
	void Interpolation(float* D);

	//Remove the peak... modified by ruixu
	void peak(float* D, int con_point_one_dir, float data);

	//Interpolation... modified by ruixu
	void Interpolation2(uint8_t* I1_desc, uint8_t* I2_desc, float* D , float* D_temp);
	float PaddingCost(const int32_t &u, const int32_t &v, uint8_t* I1_desc, uint8_t* I2_desc, int32_t* data, int32_t counts);

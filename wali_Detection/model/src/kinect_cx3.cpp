...
static const string onnx_name = "bcx3_240.onnx";
...
int main(int argc, char *argv[]) {
    ...
    // 3.init ros node
    ros::init(argc, argv, "kinect_cx3");
    ros::NodeHandle n;

    ros::ServiceClient client = n.serviceClient<kinect::RGBD_Image>("get_kinect_rgbd_image");
    ...
    while (ros::ok()) {
        if (client.call(srv)) {
            ...
            if (seq > 0) {
                ...
                cv::imshow("kinect_depth", DispRgb);

                cv::imshow("kinect_seg", seg);
                if (cv::waitKey(10) == 'q') {
                    break;
                }
            } else {
